class BIO_Player : DoomPlayer
{
		...
		double vrange = 32.0, EBFGSprayFlags flags = BFGF_NONE)
	{
		...
		double an = Angle - fireData.Angle / 2 + fireData.Angle /
		...
		if (spray != null)
		{
			...
			if ((spray.bMThruSpecies &&
				self.GetSpecies() == t.LineTarget.GetSpecies()) || 
				(!(flags & BFGF_HURTSOURCE) && self == t.LineTarget)) 
			{
			...
			if (spray.bPuffGetsOwner) spray.Target = self;
		...
		int newdam = t.LineTarget.DamageMObj(
			null, self, fireData.Damage, dmgType,
			dmgFlags | DMG_USEANGLE, t.AngleFromSource);
		t.TraceBleed(newdam > 0 ? newdam : fireData.Damage, self);
		return ret;
	}
}
